...
function _G.get_cache_dir()
  ...
  return ufovim_cache_dir
end

---Get currently installed version of ufovim
---@param type string can be "short"
---@return string
function _G.get_version(type)
  type = type or ""
  local ufovim_full_ver = vim.fn.system("git -C " .. get_runtime_dir() .. "/ufovim describe --tags")

  if string.match(ufovim_full_ver, "%d") == nil then
    return nil
  end
  if type == "short" then
    return vim.fn.split(ufovim_full_ver, "-")[1]
  else
    return string.sub(ufovim_full_ver, 1, #ufovim_full_ver - 1)
  end
end
...
function M:init()
  ...
  self.repo_dir = join_paths(self.runtime_dir, "ufovim")
...
local function git_cmd(subcmd)
  ...
  local repo_dir = join_paths(get_runtime_dir(), "ufovim")
...
function M:update_repo()
  ...
  if ret == 0 then
    Log:info "ufovim is already up-to-date"
    return
  end
