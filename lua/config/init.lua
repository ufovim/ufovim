...
--- Initialize ufovim default configuration
-- Define ufovim global variable
function M:init(opts)
  ...
  local ufovim_lsp_config = require "lsp.config"
  ufovim.lsp = vim.deepcopy(ufovim_lsp_config)
...
local function deprecation_notice()
  ...
  for lang, entry in pairs(ufovim.lang) do
    local deprecated_config = entry["lsp"] or {}
    if not vim.tbl_isempty(deprecated_config) then
      local msg = string.format(
        "Deprecation notice: [ufovim.lang.%s.lsp] setting is no longer supported. See https://github.com/ufovim/ufovim#breaking-changes",
        lang
      )
...
function M:load(config_path)
  ...
  autocmds.define_augroups(ufovim.autocommands)
