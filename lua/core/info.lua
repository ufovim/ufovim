...
local function get_formatter_suggestion_msg(ft)
  ...
  if not vim.tbl_isempty(supported_formatters) then
    vim.list_extend(section, {
      ...
      "",
      fmt("  ufovim.lang.%s.formatters = { { exe = '%s' } }", ft, table.concat(supported_formatters, "│")),
    })
  end
...
local function get_linter_suggestion_msg(ft)
  ...
  if not vim.tbl_isempty(supported_linters) then
    vim.list_extend(section, {
      ...
      "",
      fmt("  ufovim.lang.%s.linters = { { exe = '%s' } }", ft, table.concat(supported_linters, "│")),
    })
  end
...
function M.toggle_popup(ft)
  ...
  local function set_syntax_hl()
    vim.cmd [[highlight ufovimInfoIdentifier gui=bold]]
    vim.cmd [[highlight link ufovimInfoHeader Type]]
    vim.cmd [[let m=matchadd("ufovimInfoHeader", "Language Server Protocol (LSP) info")]]
    vim.cmd [[let m=matchadd("ufovimInfoHeader", "Formatters and linters")]]
    vim.cmd('let m=matchadd("ufovimInfoIdentifier", " ' .. ft .. '$")')
    vim.cmd 'let m=matchadd("string", "true")'
    vim.cmd 'let m=matchadd("error", "false")'
    tbl_set_highlight(registered_providers, "ufovimInfoIdentifier")
    -- tbl_set_highlight(require("lsp.null-ls.formatters").list_available(ft), "ufovimInfoIdentifier")
    -- tbl_set_highlight(require("lsp.null-ls.linters").list_available(ft), "ufovimInfoIdentifier")
  end
